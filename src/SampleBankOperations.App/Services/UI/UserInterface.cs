...
namespace SampleBankOperations.App.Services.UI
{
    ...
    public class UserInterface
    {
        ...
        internal void ExitApplication(Action? exitAction = null)
        {
            Console.WriteLine("Obrigado por utilizar o SampleBankOperations!");

            exitAction?.Invoke();

            if (exitAction == null && !IsRunningTest())
            {
                Environment.Exit(0);
            }
        }
        ...
        {
            return AppDomain.CurrentDomain.GetAssemblies().Any(a => a.FullName.StartsWith("xunit"));
        }
    }
}
